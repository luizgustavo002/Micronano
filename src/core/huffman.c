...
Status new_huffman_node(Huffman_Node **node, Huffman_Node *left_node, Huffman_Node *right_node) {
    ...
    if (left_node != NULL && right_node != NULL) new_node->frequency = left_node->frequency + right_node->frequency;
    else
...
uint8_t get_height_tree(Huffman_Node *root) {
    ...
    if (root->left) left = get_height_tree(root->left) + 1;
    if (root->right) right = get_height_tree(root->right) + 1;
