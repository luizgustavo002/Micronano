...
// Opens a file and assigns the provided pointer.
// - The caller is responsible for closeing the file handle.
...
Status open_file(FILE **file, const char *path, const char *mode) {
    ...
    if (file == NULL || path == NULL || mode == NULL) {
        char msg[128] = {0};
        if (file == NULL) strcat(msg, "file ");
        if (path == NULL) strcat(msg, "path ");
        if (mode == NULL) strcat(msg, "mode ");
        log_message(LOG_ERROR, "open_file invalid args: %s", msg);
    ...
    *file = NULL;
    ...
    if (path_len == 0) {
        log_message(LOG_ERROR, "Failed to convert path to UTF-16 (error: %lu).", GetLastError());
        return ERROR_INVALID_ARGUMENT;
    }
    wchar_t *wpath = (wchar_t *)malloc((size_t)path_len * sizeof(wchar_t));
    if (wpath == NULL) {
        log_message(LOG_ERROR, "Failed to allocate memory for wchar_t.");
    ...
    if (mode_len == 0) {
        log_message(LOG_ERROR, "Failed to convert mode to UTF-16 (error: %lu).", GetLastError());
        free(wpath);
        return ERROR_INVALID_ARGUMENT;
    }
    ...
    *file = _wfopen(wpath, wmode);
    ...
    free(wmode);
#else
    // --- LINUX / MAC / UNIX (Nativo)
    *file = fopen(path, mode);
#endif

    if ((*file) == NULL) {
        char *error_msg = strerror(errno);
        log_message(LOG_ERROR, "Could not open file '%s': %s (errno: %d)", path, error_msg, errno);
        if (errno == ENOENT) return ERROR_FILE_DOES_NOT_EXIST;
        if (errno == EACCES) return ERROR_PERMISSION_DENIED;
...
static Status calculate_file_size(uint64_t *size, const char *path_file) {
    ...
    if (size == NULL || path_file == NULL) {
        char msg[128] = {0};
        if (size == NULL) strcat(msg, "size ");
        if (path_file == NULL) strcat(msg, "path_file ");
        log_message(LOG_ERROR, "calculate_file_size invalid args: %s", msg);
        return ERROR_INVALID_ARGUMENT;
    }
    ...
    if (error == -1) {
        log_message(LOG_ERROR, "fseek failed in calculate_file_size");
        fclose(file);
        return ERROR_GENERIC;
    }
    long aux_size = ftell(file);
    if (aux_size < 0) {
        log_message(LOG_ERROR, "ftell failed in calculate_file_size");
        fclose(file);
        return ERROR_GENERIC;
    }
    *size = aux_size;
...
Status set_base_path(char **base_path, const char *path) {
    *base_path = (char *)calloc(strlen(path) + 1, sizeof(char));
    ...
    if (aux_path == NULL || strlen(path) <= 3) return STATUS_OK;
#else
    aux_path = strrchr(*base_path, '/');
    if (aux_path == NULL || strlen(path) <= 1) return STATUS_OK;
#endif
...
void free_file_header(File_Header **file_header) {
    ...
    if ((*file_header)->relative_path != NULL) free((*file_header)->relative_path);
...
Status read_file_header(Huffman_Decoder *decoder) {
    ...
    snprintf(absolute_path, sizeof(absolute_path), "%s%c%s", decoder->output_path, PATH_SEP,
...
Status log_compressing_file(File_List *list) {
    if (list == NULL) {
        log_message(LOG_ERROR, "open_file invalid args: list");
        return ERROR_INVALID_ARGUMENT;
    }

    size_t current_path = list->current_path;
    size_t count = list->count;
    log_message(LOG_DEBUG, "[%lu/%lu] Compressing: %s", current_path, count, list->relative_paths[current_path - 1]);
    return STATUS_OK;
}

//--------------- Get data ---------------
uint16_t get_path_size(File_Header *file_header) { return file_header->path_size; }

uint64_t get_file_size(File_Header *file_header) { return file_header->file_size; }

char *get_base_path(File_List *list) { return list->base_path; }
...
int get_is_directory(File_Header *file_header) { return file_header->is_directory; }

void get_relative_path(char *path, File_Header *file_header) { strcpy(path, file_header->relative_path); }
